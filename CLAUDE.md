...
### 핵심 시그니처
- **홈(2026-06-12~)**: HOT 블랙박스 + 카테고리 도트, 토스형 화이트 카드 피드(칩·2줄 클램프), sticky 티커
- 기사·뉴스 페이지(기존): 카테고리 도트 + monospace 라벨, ::before 컬러 스트라이프
- 동적 시각 갱신 (1분 setInterval, `[data-pubdate]` + `[data-fresh-since]`)
- 가격 1분 폴링 (`api.teslabriefing.com`) + 시세모순 핫뉴스 숨김(`data-price-dir`)
- 핫뉴스 `hot` 점수 정렬 (0~10) vs 최신뉴스 시간순 — **차별화 명확**
- Archive 누적 100개 cap (slug 기준 dedup)

## 사이트 구조 — 메인 = 토스형 모바일 카드 피드 (2026-06-12 B′ 리디자인, KO `/` + EN `/en/` 동일 템플릿)
...
- 데스크톱 = 440px 칼럼 중앙 (스펙 P0). 영상 섹션 폐지(채널 카드로 대체)
- **전체 페이지 토스형 통일 완료(2026-06-12)**: 홈·전체뉴스(`news.html`)·기사상세(`articles/*.html`) 모두 동일 시스템 — Pretendard 단일 폰트, 440 칼럼+`.page`, 하단 탭바, 카테고리 칩·강조 심볼색, 등락 미국식. Wanted Sans·IBM Plex Mono·EN Newsreader 전부 제거
- 참조 스펙: 다른 세션 산출 `redesign-spec-toss-feed.md` (JP가 B′ 변형 확정: D-day 칩 제거, 핫뉴스·티커는 현재형 유지)

### 별도 페이지
- `news.html` — archive 전체 (최대 50장)
- `articles/<slug>.html` — 카드 상세
- `privacy.html` — 개인정보처리방침 (PIPA, 라이트 톤)
- `earnings/<slug>.html` — **어닝콜 특별 페이지** (2026-07-18 신설). 데이터: `data/earnings/<slug>.json` 상태머신 `draft`(출력 0)→`upcoming`(프리뷰+D-N 핀)→`live`(전체+발표 후 7일 핀, 핀만 자동 만료·페이지 영구). 템플릿 `earnings-template.html`, 렌더 `renderEarningsPage()`. 분기마다 json 만 새로 만들면 재사용. `preQuestions[]` 필드 = Say 투자자·언론 사전 질문(upcoming/live 공통 노출). sources 는 `name_en` 병기 필수(영문 페이지 한글 누수 방지). 페이지 하단 "지난 어닝콜" 아카이브 섹션이 발행된 분기끼리 자동 상호 링크(`renderEaPastCalls`). 현재: `2026-q1`(live·발행), `2026-q2`(upcoming·프리뷰 발행 — 7/23 09:00 KST 일회성 scheduled task 가 live 전환)
