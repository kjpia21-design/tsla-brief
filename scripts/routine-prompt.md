...
```jsonc
{
  ...
  "hotShort": "시킹알파, 테슬라 핵심 성장 엔진에 타격 경고",  // 핫뉴스 모바일 1줄용 — 26자 내외 완결형(아래 규칙)
...
- 📱 **hotShort (핫뉴스 모바일 1줄)**: 핫뉴스 리스트가 모바일에서 한 줄에 다 보이도록 쓰는 **완결된 짧은 제목**.
  - **공백 포함 26자 내외**(최대 30자). 모바일 1줄에 꽉 차되 안 잘리게(짧게 끝내지 말고 핵심 맥락까지 담을 것).
  - **완결형** — 어정쩡하게 끝나지 말 것. title 을 그냥 자르지 말고 **핵심 주체 + 핵심 사실(+ 맥락 1개)**로 압축.
  - `<em>` 등 마크업 없이 평문. 예) title "JP모건, 약세 애널리스트 교체 후 테슬라에 <em>전향적 시각으로 전환</em>"
    → hotShort **"JP모건, 약세 전망 접고 테슬라에 전향적 시각"**. (❌ "JP모건, 약세 애널리스트 교체 후" 처럼 자르지 말 것)
